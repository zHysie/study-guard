...
- **浏览器扩展**：Chrome/Edge 扩展通过 Native Messaging 将当前标签页 URL 发送给桌面端。
...
> `npx tauri build` 会自动编译前端和 Rust 后端，并打包 NSIS 安装程序。原生消息主机 (`study_guardian_native_host.exe`) 也会一并编译并包含在安装包中。
...
5. 以管理员身份运行 PowerShell，注册 Native Messaging Host：

```powershell
.\scripts\install_native_messaging.ps1 -ExtensionId "你的扩展ID" -Browser Both
```

Chrome 和 Edge 扩展 ID 不同时分别执行：

```powershell
.\scripts\install_native_messaging.ps1 -ExtensionId "Chrome扩展ID" -Browser Chrome
.\scripts\install_native_messaging.ps1 -ExtensionId "Edge扩展ID" -Browser Edge
```

6. 重启浏览器，启动学习守门员。切换标签页后状态面板会显示当前 URL。
